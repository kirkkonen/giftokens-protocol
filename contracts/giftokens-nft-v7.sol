...
import "./giftokens-erc20.sol";
import 'hardhat/console.sol';

contract GiftokensNFTV7 is Ownable, ERC721URIStorage {
    using EnumerableSet for EnumerableSet.UintSet;
    using EnumerableSet for EnumerableSet.AddressSet;
    using SafeERC20 for ERC20;

    address public _relayer;
    GiftokensERC20 public _giftokenERC20;

    constructor(address relayer_, address token_) ERC721("Giftoken NFTs with balances", "GIFTOKENS") {
        _relayer = relayer_;
    ...
    function mintAndAttach(
        ...
        if(tokenIds.contains(_tokenId)) {
            ...
            //mint erc20 and approve to relayer
            _giftokenERC20.mint(address(this), _amount);
            _giftokenERC20.approve(staking, _amount);

            //attach to the minted NFT
            Contribution memory c = Contribution({
                contributor: msg.sender,
                amount: _amount
            });
            t.contributionMapping[address(_giftokenERC20)].push(c);
    ...
    function claimFunds(
        ...
        if(t.beneficiary == address(0)){
            t.beneficiary = payable(beneficiary);
        }
